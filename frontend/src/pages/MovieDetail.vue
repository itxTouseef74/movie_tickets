    ...
    <div class="max-w-full">
      <div class="mx-12" v-if="currentStep === 0">
        <div class="flex justify-center mt-7">
          <div class="bg-white shadow-2xl rounded p-1">
            <img :src="movieDoc.poster"
              alt="Movie Poster" class="w-80 h-70 object-cover rounded" />
          </div>
        </div>
        ...
        <div class="flex flex-col space-y-3 mt-14">
          ...
          <h2 class="text-gray-600 text-lg font-normal">
            {{ movieDoc.synopsis }}
          </h2>
        </div>
      </div>
      ...
      <div v-else-if="currentStep === 1">
        ...
        <div class="flex flex-col w-full space-y-5 mt-6">
          <Button size="lg" :variant="index === bookingData.numberOfSeats ? 'subtle' : 'white'" class="shadow-lg"
            v-for="index in 8" @click="setNumberOfSeats(index)">{{ index }}</Button>
        </div>
      </div>
        ...
        <div class="flex flex-col space-y-4">
          ...
          <div class="space-y-2">
            <div class="bg-white shadow-xl p-4 rounded flex flex-col space-y-4"
              v-for="theater in Object.keys(theaters.data)" :key="theater.name">
              <h3 class="text-sm font-bold text-gray-800">{{ theater }}</h3>

              <div class="flex flex-row space-x-2">
                <Button size="sm" :variant="show.name === bookingData.show ? 'subtle' : 'outline'"
                  @click="bookingData.show = show.name" :key="show.name"
                  v-for="show in theaters.data[theater]">{{ show.start_time }}</Button>
      ...
      <div v-else-if="currentStep === 3">
        ...
        <div class="w-full flex items-center flex-col mt-7">
          <div v-for="(seats, row) in seatStructure" :key="row" class="flex flex-row">
            <span @click="selectSeat(row, seat[0])" v-for="(seat, index) in seats" :key="index"
...
const movieBooking = createListResource({doctype:'Ticket Booking' ,  insert: {onSuccess(){
